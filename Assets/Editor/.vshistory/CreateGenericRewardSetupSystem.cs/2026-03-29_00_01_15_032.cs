...
");
    ...
    public static void CreateRewardAsset()
    {
        ...
        if (assetObject == null)
        {
            ScriptableObject newAsset = ScriptableObject.CreateInstance("NpcRewardProfile");
            if (newAsset == null)
            {
                Debug.LogError("NpcRewardProfile type not available yet. Run Step 1 and wait for compile.");
                return;
            }

            AssetDatabase.CreateAsset(newAsset, RewardAssetPath);
        ...
        {
            Debug.LogError("Reward asset fields not found. Make sure Step 1 compiled successfully.");
            return;
        }
