...
public class IntroMessageTrigger : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        // Așteaptă până când MessageManager este disponibil
        yield return new WaitUntil(() => FindObjectOfType<MessageManager>() != null);
    ...
    void SendIntroMessage()
    {
        if (PlayerPrefs.GetInt("IntroMessageSent", 0) == 1)
        {
            Debug.Log("⛔ [IntroTrigger] Mesajul a fost deja trimis. Nu trimitem din nou.");
            return;
        }
        ...
        Debug.Log("✅ Mesaj adăugat. Total mesaje: " + convo.messages.Count);

        mm.ShowConversations();
