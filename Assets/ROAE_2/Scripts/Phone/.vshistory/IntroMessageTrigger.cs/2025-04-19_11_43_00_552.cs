...
public class IntroMessageTrigger : MonoBehaviour
{
    ...
    public float delay = 5f; // Delay în secunde până la trimiterea mesajului
    ...
    private IEnumerator DelayedSend()
    {
        ...
        {
            elapsed += Time.unscaledDeltaTime; // CONTOR REAL, indiferent de timeScale
            yield return null;
        }
    ...
    void SendIntroMessage()
    {
        ...
        if (mm != null)
        {
            ...
            {
                mm.ShowConversationMessages(convo); // dacă telefonul e deja deschis, arătăm mesajul
            }
            else
            {
                mm.ShowConversations(); // forțăm refresh-ul inboxului
            }
