...
public class PhoneManager : MonoBehaviour
{
    ...
    public void ReceiveMessage(PhoneMessage msg)
    {
        GameObject bubble = Instantiate(messageBubblePrefab, messageParent);

        // Găsește cele două câmpuri din prefab
        TextMeshProUGUI[] texts = bubble.GetComponentsInChildren<TextMeshProUGUI>();
        foreach (var txt in texts)
        {
            if (txt.name == "SenderText")
                txt.text = msg.sender;
            else if (txt.name == "MessageText")
                txt.text = msg.content;
        }

        // TODO: adaugă diferite culori / stiluri pt mesaje vechi sau cu opțiuni
    }
