...
public class PhoneManager : MonoBehaviour
{
    ...
    {
        // 🔄 curăță lista de butoane de conversație în mod sigur
        ...
        foreach (var convo in conversations)
        {
            ...
            // ⚠️ Captură corectă a valorii locale (altfel toate butoanele apelează ultima conversație!)
            PhoneConversation capturedConvo = convo;
            button.GetComponent<UnityEngine.UI.Button>().onClick.AddListener(() => {
                ShowConversationMessages(capturedConvo);

                // 🧽 opțional: ascunde lista conversațiilor dacă vrei să treci pe ecranul de mesaje
                conversationListParent.gameObject.SetActive(false);
                messageParent.gameObject.SetActive(true);
            });
        }

        // 🔛 afișează lista conversațiilor
