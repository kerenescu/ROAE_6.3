...
public class PhoneManager : MonoBehaviour
{
    ...
    {
        // 1. Curăță mesajele vechi
        for (int i = messageParent.childCount - 1; i >= 0; i--)
        {
            Destroy(messageParent.GetChild(i).gameObject);
        }

        // 2. Copie locală a listei pentru siguranță
        var messagesCopy = new List<PhoneMessage>(convo.messages);

        // 3. Afișează fiecare mesaj fără a modifica lista originală
        foreach (var msg in messagesCopy)
        {
            GameObject bubble = Instantiate(messageBubblePrefab, messageParent);

            // 👉 Aici adaugi verificarea cine e senderul
            bool isFromRina = msg.sender == "Rina";

            // 🔀 Ia RectTransform-ul ca să modifici ancorarea
            RectTransform bubbleRect = bubble.GetComponent<RectTransform>();
            bubbleRect.pivot = new Vector2(isFromRina ? 1 : 0, 0.5f);
            bubbleRect.anchorMin = new Vector2(isFromRina ? 1 : 0, 1);
            bubbleRect.anchorMax = new Vector2(isFromRina ? 1 : 0, 1);
            bubbleRect.anchoredPosition = new Vector2(isFromRina ? -30 : 30, 0); // ușor distanțat vizual

            // 🔠 CONTINUĂ cu partea ta care setează textul etc.
            TextMeshProUGUI[] texts = bubble.GetComponentsInChildren<TextMeshProUGUI>();
            if (texts.Length >= 2)
            {
                ...
                else
                {
                    texts[0].fontStyle = FontStyles.Bold;
                    texts[1].fontStyle = FontStyles.Normal;
                    bubble.GetComponent<Image>().color = isFromRina ? new Color(0.85f, 1f, 0.85f) : Color.white;
                }
            }

        }

        // 4. Schimbă vizibilitatea corect
