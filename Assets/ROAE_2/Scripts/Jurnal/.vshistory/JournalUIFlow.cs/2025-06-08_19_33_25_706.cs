...
public class JournalUIFlow : MonoBehaviour
{
    ...
    public JournalPageData[] journalPages;
    private int currentPageIndex = 0;

    public void NextPage()
    {
        currentPageIndex = (currentPageIndex + 1) % journalPages.Length;
    ...
    {
        currentPageIndex--;
        if (currentPageIndex < 0) currentPageIndex = journalPages.Length - 1;
        currentPage = journalPages[currentPageIndex];
        DisplayJournalContent();
    }
    ...
    JournalUIFlow.Instance.AddPageIfNotPresent(pageLichenia);
