...
public class TarotReadingManager : MonoBehaviour
{
    ...
    [SerializeField] private Collider2D physicalDeckCollider;
    public JournalEntryTrigger tarotJournalEntryTrigger;
    ...
    public void StartReading()
    {
        ...
        if (physicalDeckCollider != null)
            physicalDeckCollider.enabled = false;
        ...
        // dezactivează cursorul și inputul Adventure Creator
        AC.PlayerInput playerInput = FindObjectOfType<AC.PlayerInput>();
        if (playerInput != null)
        {
            playerInput.enabled = false;
        }

        AC.PlayerCursor cursor = FindObjectOfType<AC.PlayerCursor>();
        if (cursor != null)
        {
            cursor.enabled = false;
        }

        // opțional: oprește animații sau sunete dacă ai personaje în mișcare
        ...
        if (isRepeatReading)
        {
            ...
            for (int i = 0; i < maxCards; i++)
            {
                ...
                // poziționează instant în slotul de reveal
                if (i < revealPositions.Count)
                {
                    card.transform.SetParent(revealPositions[i]);
                    card.transform.localPosition = Vector3.zero;
                    card.transform.localScale = Vector3.one;
                }
            }

            // dacă vrei să arăți și interpretările (opțional):
            // foreach (var card in selectedCards)
            //     ShowInterpretation(card.GetComponent<TarotCard>().GetInterpretation()); ← dacă ai expus-o

            // sari direct la linia finală
    ...
    private void CloseReading()
    {
        ...
        // REACTIVĂM toate hotspoturile
        foreach (AC.Hotspot h in FindObjectsOfType<AC.Hotspot>())
        {
            h.enabled = true;
        }

        // REACTIVĂM cursorul și inputul AC
        AC.PlayerInput playerInput = FindObjectOfType<AC.PlayerInput>();
        if (playerInput != null)
        {
    ...
    {
        return tarotPanelUI.activeSelf;
    }
    ...
    if (tarotJournalEntryTrigger != null)
{
    tarotJournalEntryTrigger.TryAddPage();
}
