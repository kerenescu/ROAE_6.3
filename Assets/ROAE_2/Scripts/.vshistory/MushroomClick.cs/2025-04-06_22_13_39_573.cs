...
public class MushroomWithTextUI : MonoBehaviour
{
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (hit != null && hit.gameObject == this.gameObject)
            {
                ...
                if (clickTexts.Length > 0 && typewriterEffect != null)
                    typewriterEffect.Run(clickTexts[currentSpriteIndex]);

                clickIndex++;

                // Verificăm dacă această ciupercă este în starea sa magică
                isInMagicState = (currentSpriteIndex == targetIndex);

                // Verificăm toate ciupercile
    ...
    void VerificaToateCiupercile()
    {
        ...
        // TODO: aici adaugi efectul real, gen:
        FindObjectOfType<GlowEffectController>().TriggerVignette();
